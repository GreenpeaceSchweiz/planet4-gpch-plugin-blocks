...
if ( ! class_exists( '\P4CHBKS\Loader' ) ) {
	...
	final class Loader {
		...
		private $required_php = P4CHBKS_REQUIRED_PHP;
		/** @var array $required_plugins */
		private $required_plugins = P4CHBKS_REQUIRED_PLUGINS;
		...
		private function hook_plugin() {
			...
			do_action( 'P4CHBKS_action_loaded' );
		}
			...
			if ( is_admin() ) {         // If we are on the admin panel.
				...
				if ( $this->check_required_php() ) {
					...
					if ( $this->check_required_plugins( $plugins ) ) {
						...
						deactivate_plugins( P4CHBKS_PLUGIN_BASENAME );
						$count = 0;
						$message = '<div class="error fade">' .
									'<u>' . esc_html( P4CHBKS_PLUGIN_NAME ) . ' > ' . esc_html__( 'Requirements Error(s)', 'planet4-gpch-blocks' ) . '</u><br /><br />';

						foreach ( $plugins['not_found'] as $plugin ) {
							$message .= '<br/><strong>' . (++$count) . '. ' . esc_html( $plugin['Name'] ) . '</strong> ' . esc_html__( 'plugin needs to be installed and activated.', 'planet4-gpch-blocks' ) . '<br />';
						}
						foreach ( $plugins['not_updated'] as $plugin ) {
							$message .= '<br/><strong>' . (++$count) . '. ' . esc_html( $plugin['Name'] ) . '</strong><br />' .
										esc_html__( 'Minimum version ', 'planet4-gpch-blocks' ) . '<strong>' . esc_html( $plugin['min_version'] ) . '</strong>' .
										'<br/>' . esc_html__( 'Current version ', 'planet4-gpch-blocks' ) . '<strong>' . esc_html( $plugin['Version'] ) . '</strong><br />';
						}
					...
					wp_die(
						...
						'<strong>' . esc_html__( 'PHP Requirements Error', 'planet4-gpch-blocks' ) . '</strong><br /><br />' . esc_html( P4CHBKS_PLUGIN_NAME . __( ' requires a newer version of PHP.', 'planet4-gpch-blocks' ) ) . '<br />' .
						'<br/>' . esc_html__( 'Minimum required version of PHP: ', 'planet4-gpch-blocks' ) . '<strong>' . esc_html( $this->required_php ) . '</strong>' .
						'<br/>' . esc_html__( 'Running version of PHP: ', 'planet4-gpch-blocks' ) . '<strong>' . esc_html( phpversion() ) . '</strong>' .
		...
		public function load_admin_assets( $hook ) {
			...
			if ( strpos( $hook, P4CHBKS_PLUGIN_SLUG_NAME ) === false ) {
				return;
			}

			wp_enqueue_script( 'P4CHBKS_admin_jquery', '//code.jquery.com/jquery-3.2.1.min.js', array(), '3.2.1', true );
			wp_enqueue_style( 'P4CHBKS_admin_style', P4CHBKS_ADMIN_DIR . 'css/admin.css', array(), '0.1' );
			wp_enqueue_script( 'P4CHBKS_admin_script', P4CHBKS_ADMIN_DIR . 'js/admin.js', array(), '0.1', true );
		}
		...
		public function load_i18n() {
			load_plugin_textdomain( 'planet4-gpch-blocks', false, P4CHBKS_PLUGIN_DIRNAME . '/languages/' );
		}
	...
	wp_die(
		'<div class="error fade">' .
		'<u>' . esc_html( P4CHBKS_PLUGIN_NAME ) . esc_html__( 'Conflict Error', 'planet4-gpch-blocks' ) . '</u><br /><br />' . esc_html__( 'Class P4CHBKS_Loader already exists.', 'planet4-gpch-blocks' ) . '<br />' .
